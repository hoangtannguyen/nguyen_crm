    ...
    <section class="content">
        <div class="container">
            <div class="head">
                ...
                <h1 class="title">{{ __('Sửa Dự Án') }}</h1>
            </div>
            <div class="main">
                @include('notices.index')
                <form action="{{ route('project.put' , $projects->id)}}" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-9">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" value="{{ $projects->title }}" class="form-control" data-error="{{ __('Vui lòng nhập tên dự án')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Nguồn mua sắm') }} <small>({{ __('require') }})</small></label>
                                <input type="text" name="procurement" value="{{ $projects->procurement }}" class="form-control" data-error="{{ __('Vui lòng nhập nguồn mua sắm')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Quyết định') }} <small>({{ __('require') }})</small></label>
                                <input type="number " name="decision" value="{{ $projects->decision }}" class="form-control" data-error="{{ __('Vui lòng nhập quyết định')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Ghi chú') }} <small>({{ __('require') }})</small></label>
                                <input type="text" name="note" value="{{ $projects->note }}" class="form-control" data-error="{{ __('Vui lòng nhập ghi chú')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Trạng thái') }} <small>({{ __('require') }})</small></label>
                                    <select class="form-control select2"  name="status">
                                        @foreach ($get_statusProjects as $key => $item)
                                            <option  {{ $key == $projects->status ? 'selected':'' }} value="{{ $key }}">{{ $item }}</option>
                                        @endforeach
                                    </select>
                            </div>
                            <div class="form-group">
                              <label>{{ __('Từ ngày') }}<small>({{ __('require') }})</small></label>
                                <div class="input-group date" id="reservationdate" data-target-input="nearest">
                                    <input name="fromDate"  value="{{ $projects->fromDate }}"  type="text" class="form-control datetimepicker-input" data-target="#reservationdate"/ data-error="{{ __('Vui lòng nhập từ ngày')}}" required>
                            ...
                            <div class="form-group">
                              <label>{{ __('Đến ngày') }}<small>({{ __('require') }})</small></label>
                                <div class="input-group date" id="reservationdate_to" data-target-input="nearest">
                                    <input name="toDate" value="{{ $projects->toDate }}" type="text" class="form-control datetimepicker-input" data-target="#reservationdate_to"/  data-error="{{ __('Vui lòng nhập đến ngày')}}" required>
                                ...
                                <div class="card-body">
                                    <div id="frm-avatar" class="img-upload">
                                        <div class="image">
                                            ...
                                            {!! image($projects->image,230,230,__('Avatar')) !!}
                                            <input type="hidden" name="image" class="thumb-media" value="{{ $projects->image }}" />
                                        </div>
                                    </div>
                                </div>
                            </aside>
