...
@section('title',__('Sửa - Sửa Chữa Thiết Bị'))
@section('content')
@php 
    ...
    <section class="content">
        <div class="container">
            <div class="head">
                ...
                <h1 class="title">{{ __('Sửa - Sửa Chữa Thiết Bị') }}</h1>
            </div>
            <div class="main">
                @include('notices.index')
                <form action="{{ route('eqrepair.put' , $eqrepairs->id)}}" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-9">
                                ...
                                <label class="control-label">{{ __('Tên thiết bị') }} <small>({{ __('require') }})</small></label>
                                    <select class="form-control select2"  name="equi_id">
                                        @foreach ($equipments as $equipment)
                                            <option  {{ $equipment->id == $eqrepairs->equi_id ? 'selected':'' }}  value="{{ $equipment->id }}">{{ $equipment->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Mã hiển thị') }} <small>({{ __('require') }})</small></label>
                                <input type="text" name="code" value="{{ $eqrepairs->code }}" class="form-control" data-error="{{ __('Vui lòng nhập mã hiển thị')}}" required>
                            ...
                            <label class="control-label">{{ __('Người nhập') }} <small>({{ __('require') }})</small></label>
                                <select class="form-control select2"  name="user_id">
                                    @foreach ($users as $user)
                                        <option  {{ $user->id == $eqrepairs->user_id ? 'selected':'' }} value="{{ $user->id }}">{{ $user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Lý do') }} <small>({{ __('require') }})</small></label>
                                <textarea name="reason" class="editor form-control" id="" cols="30" rows="10" >{{ $eqrepairs->reason }}</textarea>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Nội dung') }} <small>({{ __('require') }})</small></label>
                                <textarea name="content" class="editor form-control" id="" cols="30" rows="10">{{ $eqrepairs->content }}</textarea>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Trạng thái') }} <small>({{ __('require') }})</small></label>
                                    <select class="form-control select2"  name="status">
                                        @foreach ($get_statusAction as $key => $items)
                                            <option  {{ $key == $eqrepairs->status ? 'selected':'' }} value="{{ $key }}">{{ $items }}</option>
                                        @endforeach
                                    </select>
                            </div>
                                    ...
                                    <div class="card-body">
                                        <div id="frm-avatar" class="img-upload">
                                            <div class="image">
                                                ...
                                                {!! image($eqrepairs->image,230,230,__('Avatar')) !!}
                                                <input type="hidden" name="image" class="thumb-media" value="{{ $eqrepairs->image }}" />
                                            </div>
                                        </div>
                                    </div>
                            </aside>
