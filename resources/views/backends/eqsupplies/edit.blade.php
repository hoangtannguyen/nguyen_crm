...
@section('title',__('Sửa Chi Tiết Vật Tư'))
@section('content')
@php 
    ...
    <section class="content">
        <div class="container">
            <div class="head">
                ...
                <h1 class="title">{{ __('Sửa Chi Tiết Vật Tư') }}</h1>
            </div>
            <div class="main">
                @include('notices.index')
                <form action="{{ route('eqsupplie.put' , $equipments->id)}}" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-9">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" value="{{ $equipments->title }}" class="form-control" data-error="{{ __('Vui lòng nhập tiêu đề hiển thị')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Loại vật tư') }} <small>({{ __('require') }})</small></label>
                                <select class="select2 form-control" name="equipment_supplie[]" multiple="multiple"  >
                                    @foreach ($supplies as $supplie)
                                        <option {{ in_array($supplie->id, $array) ? ' selected' : '' }} value="{{$supplie->id}}">{{$supplie->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <label class="control-label">{{ __('Người nhập') }} <small>({{ __('require') }})</small></label>
                                    <select class="form-control select2"  name="user_id">
                                        @foreach ($users as $user)
                                            <option {{ $user->id == $equipments->user_id ? 'selected':'' }} value="{{ $user->id }}">{{ $user->name }}</option>
                                        @endforeach
                                    </select>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control select2"  name="status">
                                    @foreach ($statusEquipments as $key => $items)
                                        <option {{ $key == $equipments->status ? 'selected':'' }}  value="{{ $key }}">{{ $items }}</option>
                                    @endforeach
                                </select>
                                <div class="help-block with-errors"></div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="code" value="{{ $equipments->code }}" class="form-control" data-error="{{ __('Vui lòng nhập mã hiển thị')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Số serial') }} <small>({{ __('require') }})</small></label>
                                <input type="number " name="serial" value="{{ $equipments->serial }}" class="form-control" data-error="{{ __('Vui lòng nhập số serial')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
